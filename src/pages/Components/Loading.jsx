...
// Loading component
const Loading = ({ text, useSmall, useAlt }) => {
    return useSmall != null && useSmall ? (
        <p className={`text-center font-semibold ${useAlt ? "text-white" : "text-tec-darker"}`}>
            <FaCircleNotch className="w-6 h-6 mr-2 motion-safe:animate-spin inline" />
            {text != null && text != "" ? text : "Loading..."}
        </p>
    ) : (
        <div className={`w-fit mx-auto ${useAlt ? "text-white" : "text-tec-darker"}`}>
